...
$pageConfig = [
    ...
    'scripts' => ["../../dashboard.js", "./officer-notification-scripts.js"],
    'authRequired' => true
];
...
if ($type === 'announcement') {
    ...
    $sql = "SELECT * FROM notifications WHERE id = ? AND reciever_id = ? AND reciever_type = 'officer' LIMIT 1";
}
...
if ($type === 'announcement') {
    ...
    $stmt->bind_param("is", $id, $officer_id);

    // Mark as read
    $update_sql = "UPDATE notifications SET is_read = 1 WHERE id = ? AND reciever_id = ? AND reciever_type = 'officer'";
    $update_stmt = $conn->prepare($update_sql);
    $update_stmt->bind_param("is", $id, $officer_id);
    $update_stmt->execute();
}
