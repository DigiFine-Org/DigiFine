...
if ($result->num_rows === 0) {
    die("Fine not found or unauthorized access.");
}
...
$sql = "
    ...
    WHERE f.id = ? AND f.offence_type = 'court' 
    AND o.police_station = (SELECT police_station FROM officers WHERE id = ?)
";
$stmt = $conn->prepare($sql);
...
if ($result->num_rows === 0) {
    die("No fine found, not of type 'court', or unauthorized access.");
}
