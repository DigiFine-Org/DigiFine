window.fetchIssuedReportedData = function () {
  ...
  fetch(
    `issued-reported/get-fines.php?police_id=${officerId}&period=${timePeriod}`
  )
    ...
    .then((data) => {
      ...
      if (data.error) {
        ...
        updateReportedChart(data, timePeriod);
...
function updateReportedChart(data, period) {
  const ctx = document.getElementById("ReportedfineChart").getContext("2d");

  if (window.reportedChart) {
    window.reportedChart.destroy();
  }
  ...
  window.reportedChart = new Chart(ctx, {
    type: "line",
    data: {
      labels: labels,
      datasets: [
