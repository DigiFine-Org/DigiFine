...
// Fetch data for the fines by offence type table
$url = "http://localhost/digifine/dashboard/admin/reports/police-station-report/revenue/offence-type-revenue/get-offences-revenue.php?police_station=" . urlencode($policeStationId) . "&time_period=" . urlencode($timePeriod);
...
// Generate fines by offence type table rows
$tableRows = '';
if (!empty($data)) {
    $rank = 1;
    foreach ($data as $row) {
...
// Download PDF
$dompdf->stream("offence_type_report_" . date('Ymd_His') . ".pdf", ["Attachment" => 1]);
exit;
