document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateChart(data, period) {
    ...
    if (period === "24h" || period === "72h") {
      ...
      // For last 365 days, display data by month using the last 12 months.
  ...
  function getLastHours(hours) {
    const labels = [];
    const now = new Date();
    // Align now to the start of the current hour:
    now.setMinutes(0, 0, 0);
    // Start time = now minus (hours - 1) hours:
    const startTime = new Date(now.getTime() - (hours - 1) * 3600 * 1000);
    for (let i = 0; i < hours; i++) {
      const tempDate = new Date(startTime.getTime() + i * 3600 * 1000);
  ...
  function getLastDays(days) {
    const labels = [];
    const now = new Date();
    // Align now to midnight
    now.setHours(0, 0, 0, 0);
    const startDate = new Date(now.getTime() - (days - 1) * 24 * 3600 * 1000);
    for (let i = 0; i < days; i++) {
      const tempDate = new Date(startDate.getTime() + i * 24 * 3600 * 1000);
      const year = tempDate.getFullYear();
      const month = String(tempDate.getMonth() + 1).padStart(2, "0");
      const day = String(tempDate.getDate()).padStart(2, "0");
