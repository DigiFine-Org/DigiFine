window.fetchOffencesFineData = function () {
  const timePeriod = document.getElementById("timePeriod").value;
  const policeStationId = document.getElementById("policeStation").value;

  fetch(
    `offence-type/get-offences.php?policeStation=${policeStationId}&time_period=${timePeriod}`
  )
...
function updateOffencesChart(data, period) {
  const ctx = document.getElementById("OffenesChart").getContext("2d");

  if (window.offenceChart) {
    window.offenceChart.destroy();
  }
  ...
  window.offenceChart = new Chart(ctx, {
    type: "bar",
    data: {
    ...
    options: {
      ...
      plugins: {
        title: {
          display: true,
          text: `Number of Fines by Offence (${period})`,
        },
        legend: {
          display: false,
        },
