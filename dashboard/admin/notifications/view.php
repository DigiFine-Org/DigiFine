...
if ($type === 'announcement') {
    ...
    $sql = "SELECT * FROM notifications WHERE id = ? AND reciever_id = ? AND reciever_type = 'admin' LIMIT 1";
}
...
if ($type === 'announcement') {
    ...
    $stmt->bind_param("is", $id, $admin_id);


    $update_sql = "UPDATE notifications SET is_read = 1 WHERE id = ? AND reciever_id = ? AND reciever_type = 'admin'";
    $update_stmt = $conn->prepare($update_sql);
    $update_stmt->bind_param("is", $id, $admin_id);
    $update_stmt->execute();
}
